...
public class EmployeeCURDTestWithSteps extends TestBase {
    ...
    static String employeeName = "Bella";
    @Steps
    EmployeeSteps dummyRestAPISteps;
    ...
    @Title("3. Verify if 24rd employee name is Doris Wilder")
    @Test
    @Order(7)
    public void test007() {
    ...
    @Title("6. Verify 21st id salary is 345000")
    @Test
    @Order(10)
    public void test010() {
        List<?> employeeSalary = dummyRestAPISteps.readingAllEmployee().log().all().extract().path("data.findAll{it.id==21}.employee_salary");
        Assert.assertEquals(employeeSalary.get(0), 345000);
    }

    @Title("7. Verify 16th id age is 66")
    @Test
    @Order(11)
    public void test011() {
        List<?> employeeAge = dummyRestAPISteps.readingAllEmployee().log().all().extract().path("data.findAll{it.id==16}.employee_age");
        Assert.assertEquals(employeeAge.get(0), 66);
    }

    @Title("8. Verify 10th id name is Sonya Frost")
    @Test
    @Order(12)
    public void test012() {
        List<?> employeeName1 = dummyRestAPISteps.readingAllEmployee().log().all().extract().path("data.findAll{it.id==11}.employee_name");
        Assert.assertEquals(employeeName1.get(0), "Sonya Frost");
    }
